...
#
# systemd ExecStart for hermes-render-worker.service on HomeD13. Fetches the
# broker token from Vaultwarden and `exec`s the worker, so the token never
# touches disk — same pattern as tools/hermes-gateway-wrapper.sh
# (IMPLEMENTATION_PLAN.md §2b).
#
# This is the only secret this node's worker needs. After Stage 3f, it is the
# only secret this node needs at all.
set -euo pipefail
...
exec /usr/bin/python3 "$REPO_DIR/tools/hermes-render-worker.py"
