...
# time (built and documented back on 2026-09-06 as "wired into Matrix chat," but never actually
# deployed until now): its own `claim_next('gameadmin')` failed with a 400 immediately on
# startup, because the `gameadmin` topic — and the `gameadmin` identity it publishes results as
# in `publish_result()` — were never registered here, the exact same "forgetting a new sender
# identity crashes its caller" class of bug this file's own 2.0.3 note already documents.
#
...
# bots, named Mark and Luke... priority towards arming themselves and defending the spawn
# point"). Same convention as `mc-babs`/`mc-amy` (2.0.16) — no new topics needed, they reuse
# `minecraft`/`minecraft-coordination`, already a 2+ bot design.
#
...
# want the Firmament to do this monitoring, and engage coder/coder2 loop to do initial triage" —
# built after a long live debugging session surfaced a real, recurring taxonomy of Minecraft bot
# failures worth automating triage for). Deliberately a topic separate from the existing
# `minecraft` — see this file's own note at KNOWN_TOPICS for why reusing it would be wrong (both
# bots relay everything they hear on `minecraft` into in-game chat).
#
...
# entry below this one documents. `dualcoder` IS a normal `hermes-dispatch.py` routing target — see
# tools/hermes-dualcoder.py.
#
